...
Result<std::vector<FileEntry>> collectFiles(const fs::path& folderPath)
{
    ...
    for (auto it = fs::recursive_directory_iterator(folderPath, fs::directory_options::skip_permission_denied, ec);
        ...
        if (it->is_regular_file(ec)) {
            ...
            entry.relativePath = fs::relative(it->path(), folderPath).generic_string();
            entry.size = static_cast<uint64_t>(fs::file_size(it->path(), ec));
                                         ...
                                         const SecureBytes& key)
{
    ...
    if (renameEc) {
        return failAndCleanup(ErrorCode::FileWriteError, "Failed to finalize unlocked folder: " + renameEc.message());
    }
